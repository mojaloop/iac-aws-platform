resource "helm_release" "mfi-account-oracle" {
  ...
  values = [
    templatefile("${path.module}/templates/values-mfi-account-oracle.yaml.tpl", {
      ingress_host = "${var.mfi_account_oracle_name}.${dependency.baseinfra.outputs.public_subdomain}"
...
output "mfi-account-oracle-fqdn" {
  ...
  value = var.use_mfi_account_oracle_endpoint == "yes" ? "${var.mfi_account_oracle_name}.${dependency.baseinfra.outputs.public_subdomain}" : "not used"
}
