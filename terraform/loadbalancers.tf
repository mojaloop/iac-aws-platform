...
resource "aws_eip" "nlb" {
  for_each = {
    for az in local.availability_zones : data.terraform_remote_state.tenant.outputs.public_subnet_ids["${var.environment}-${az}"]["id"] => data.terraform_remote_state.tenant.outputs.public_subnet_ids["${var.environment}-${az}"]
  }
...
resource "aws_lb" "internal-lb" { #  for internal traffic, including kube traffic
  ...
  subnets            = [for az in local.availability_zones : data.terraform_remote_state.tenant.outputs.private_subnet_ids["${var.environment}-${az}"]["id"]]
...
resource "aws_lb_target_group_attachment" "internal-31443" {
  for_each = {
    for i, worker in module.k8-cluster-main.worker_nodes : i => worker.id
  }
  target_group_arn = aws_lb_target_group.internal-31443.arn
...
resource "aws_lb_target_group_attachment" "internal-6443" {
  for_each = {
    for i, master in module.k8-cluster-main.master_nodes : i => master.id
  }
  target_group_arn = aws_lb_target_group.internal-6443.arn
  target_id = each.value
}