...
sed -i "s/@JWS_KEY_PASSWORD@/$JWS_KEY_PASSWORD/g" ${WORKING_DIRECTORY}/conf/*
...
#import own ca
keytool -importcert -file ./cert.pem -keystore ./client-truststore.jks -storetype JKS -storepass $KEYSTORE_PASSWORD -alias localhost -noprompt
...
#jws key pair/commenting out for now.
SIMPLE_ALIAS=switch
openssl pkcs12 -export -in jws_cert.pem -inkey jws_priv_key.pem -name $SIMPLE_ALIAS -certfile root_ca.pem -password pass:$KEYSTORE_PASSWORD -out ./switch.pfx
keytool -importkeystore -srckeystore ./switch.pfx  -srcstoretype pkcs12 -destkeystore switch.jks -deststoretype JKS -deststorepass $KEYSTORE_PASSWORD --srcstorepass $KEYSTORE_PASSWORD -destalias $SIMPLE_ALIAS -srcalias $SIMPLE_ALIAS
cp ${WORKING_DIRECTORY}/secrets/switch.jks $WSO2_SERVER_HOME/repository/resources/security/switch.jks
# custom configurations
cp ${WORKING_DIRECTORY}/conf/dfspKeyStoreConfig.xml $WSO2_SERVER_HOME/configs/FSP/dfspKeyStoreConfig.xml
cp ${WORKING_DIRECTORY}/conf/260_velocity_template.xml $WSO2_SERVER_HOME/repository/resources/api_templates/velocity_template.xml
