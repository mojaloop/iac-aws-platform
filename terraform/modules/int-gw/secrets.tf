...
resource "tls_cert_request" "jws" {
  key_algorithm   = "RSA"
  private_key_pem = tls_private_key.jws.private_key_pem
...
resource "tls_locally_signed_cert" "jws" {
  cert_request_pem   = tls_cert_request.jws.cert_request_pem
  ca_key_algorithm   = "RSA"
  ca_private_key_pem = var.root_private_key
  ca_cert_pem        = var.root_certificate
