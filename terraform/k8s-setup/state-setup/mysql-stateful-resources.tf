resource "helm_release" "mysql" {
  for_each    = {for stateful_resource in var.stateful_resources : stateful_resource.resource_name => stateful_resource if stateful_resource.local_resource != null && stateful_resource.local_resource.mysql_data != null}
...
resource "random_password" "mysql" {
  for_each    = {for stateful_resource in var.stateful_resources : stateful_resource.resource_name => stateful_resource if stateful_resource.local_resource != null && stateful_resource.local_resource.mysql_data != null && stateful_resource.local_resource.create_resource_random_password == true}
  length = 16
  special = false
}

resource "vault_generic_secret" "mysql" {
  for_each    = {for stateful_resource in var.stateful_resources : stateful_resource.resource_name => stateful_resource if stateful_resource.local_resource != null && stateful_resource.local_resource.mysql_data != null && stateful_resource.vault_credential_paths.pw_data != null}
