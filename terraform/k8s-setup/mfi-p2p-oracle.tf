resource "helm_release" "mfi-p2p-oracle" {
  ...
  values = [
    templatefile("${path.module}/templates/values-mfi-p2p-oracle.yaml.tpl", {
      ingress_host = "${var.mfi_p2p_oracle_name}.${data.terraform_remote_state.infrastructure.outputs.public_subdomain}"
...
output "mfi-p2p-oracle-fqdn" {
  ...
  value = var.use_mfi_p2p_oracle_endpoint == "yes" ? "${var.mfi_p2p_oracle_name}.${data.terraform_remote_state.infrastructure.outputs.public_subdomain}" : "not used"
}
