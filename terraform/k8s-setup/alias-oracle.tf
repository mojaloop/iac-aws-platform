resource "helm_release" "alias-oracle" {
  ...
  values = [
    templatefile("${path.module}/templates/values-alias-oracle.yaml.tpl", {
      ingress_host = "${var.alias_oracle_name}.${data.terraform_remote_state.infrastructure.outputs.public_subdomain}"
...
output "alias-oracle-fqdn" {
  ...
  value = var.use_alias_oracle_endpoint == "yes" ? "${var.alias_oracle_name}.${data.terraform_remote_state.infrastructure.outputs.public_subdomain}" : "not used"
}